...
void X11WindowAdapter::configure(quintptr windowId)
{
    ...
    sendNetWmStateMessage(window, QStringLiteral("_NET_WM_STATE_SKIP_TASKBAR"),
                          QStringLiteral("_NET_WM_STATE_SKIP_PAGER"));
    XFlush(dpy_);
}
...
void X11WindowAdapter::setOnAllDesktops(quintptr windowId, bool on)
{
    ...
    XChangeProperty(dpy_, Window(windowId), netWmDesktop, XA_CARDINAL, 32, PropModeReplace,
